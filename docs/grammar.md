...
- **`assignment` vs. `expr_stmt` both start with `IDENT`.** Unlike every other alternative in the `statement` rule (each of which starts with a unique keyword), `assignment` (`IDENT ["[" expr "]"] "=" expr`) and `expr_stmt` (`IDENT "(" args ")"`) both begin with an identifier. The first token alone can't disambiguate them — `parse_statement()` must consume the `IDENT`, then switch on the *next* token: `=` or `[` → assignment, `(` → call-as-statement. This is the same one-token-of-extra-lookahead trick `postfix` already uses, just needed one level higher up in the grammar too. Flagging this explicitly because a naive implementation that dispatches `parse_statement()` purely on the first token (like the keyword-led branches can) would silently mishandle this one case.
